...
def valida_opcao_jogador(info):
    try:
        int(info)
    except ValueError:
        return False
    return int(info) in range(3)


def nova_partida(info):
    # 1 - Sim; 2 - Não
    return info == '1'
...
while True:
    ...
    while rodada < TENTATIVAS:
        ...
        numero_jogador = get_info_jogador(MENSAGENS.get('DEFAULT').get('Info_JOGADOR'))
    ...
    opcao = get_info_jogador(MENSAGENS.get('DEFAULT').get('NOVA_PARTIDA'))
